...
knowledge delivered by a Codex-side loader. Both CLIs run independently.
...
The legacy `model` stays with Claude. `inherit` omits Codex's model key,
...
Claude tool names, turn limits, permissions, MCP declarations, and agent
...
knowledge files remain untouched. Avoid running another skill bridge that
manages the same registrations.
